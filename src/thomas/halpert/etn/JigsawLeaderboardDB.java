...
public class JigsawLeaderboardDB {
	...
	public static boolean editEntry(String id, String user, String timeToEscape)
	{
		...
		String query = "UPDATE JIGSAW SET TIME = ?, NAME = ? WHERE ID = ?";
		if(conn != null) {
			try {
	...
	public static boolean addEntry(String user, String timeToEscape)
	{
		...
		String query = "INSERT INTO JIGSAW (TIME, NAME) values (?,?)";
		if(conn != null) {
			try {
				ps = conn.prepareStatement(query);
				ps.setString(1, timeToEscape);
	...
	public static boolean removeEntry(String id)
	{
		...
		String query = "DELETE FROM JIGSAW WHERE ID = ?";
		if(conn != null) {
			try {
				ps = conn.prepareStatement(query);
				ps.setString(1, id);
